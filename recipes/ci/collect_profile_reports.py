...
def collect(folder):
    ...
    rejected=[];reports=[]
    for file in sorted(folder.glob('profile-report-*.json')):
        report=json.loads(file.read_text());engine=report.get('engine')
        if report.get('profile_bundle_sha256')!=digest:
            rejected.append({'report':file.name,'reason':'source_bundle_mismatch'});continue
        if engine not in DIALECT or not report.get('engine_version') or not report.get('cases'):
            rejected.append({'report':file.name,'reason':'invalid_or_empty_engine_report'});continue
        ...
        for cid in ids:
            ...
            by_key[(cid,DIALECT[engine])].setdefault('execution_evidence',[]).append({
                'engine':engine,'engine_version':report['engine_version'],'report':file.name,
...
if __name__=='__main__':
    ap=argparse.ArgumentParser(description=__doc__);ap.add_argument('--bundle',default='recipes/out');a=ap.parse_args()
    result=collect(a.bundle)
